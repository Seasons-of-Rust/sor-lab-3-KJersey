...
impl Shop {
    ...
    fn most_expensive(&self) -> u32 {
        // There are always three cards, so max is guarenteed to never be none
        self.cards.iter().map(|c| c.price).max().unwrap()
    }

    /// Get the total damage of all cards in the shop
    fn total_damage(&self) -> u32 {
        self.cards.iter().map(|c| c.damage).sum()
    }

    /// Get the total health of all cards in the shop
    fn total_health(&self) -> u32 {
        self.cards.iter().map(|c| c.health).sum()
    }
}
